...
class MistakeQuestionDao extends InterfaceDao {
	...
	INSERT (mistake, callback) {
		...
		const values = [mistake.questions_wrong_id, mistake.user_id, mistake.session_name];
		const qtext = `INSERT INTO user_mistake(mistake_id, questions_wrong_id, user_id, session_name) 
                    VALUES (DEFAULT, ${this.dv(values[0])}, ${this.dv(values[1])}, ${this.dv(values[2])}) 
                    RETURNING mistake_id`;
		console.log(qtext)
		pool.query(qtext)
			.then(res => {
				console.log(res);
				// Set the generated mistake_id from the database
				if (res.rows && res.rows.length > 0) {
					mistake.mistake_id = res.rows[0].mistake_id;
				}
				callback(mistake);
			})
			.catch(err => {
