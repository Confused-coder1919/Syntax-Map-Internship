...
class QuestionDao extends InterfaceDao {
	...
	INSERT (question, callback) {
		...
		const values = [questionId, getSyntaxe(question.question_title), getSyntaxe(question.answer_title_a), getSyntaxe(question.answer_title_b), getSyntaxe(question.answer_title_c), getSyntaxe(question.answer_title_d), question.right_answer, getSyntaxe(question.explanation), question.difficulty, question.online_exam_ids, question.quiz_details_id];
		const qtext = `INSERT INTO question_table(question_id, question_title, answer_title_a, answer_title_b, answer_title_c, answer_title_d, right_answer, explanation, difficulty, online_exam_ids, quiz_details_id) VALUES (${this.dv(values[0])}, ${this.dv(values[1])}, ${this.dv(values[2])}, ${this.dv(values[3])}, ${this.dv(values[4])}, ${this.dv(values[5])}, ${this.dv(values[6])}, ${this.dv(values[7])}, ${this.dv(values[8])}, ${this.dv(values[9])}, ${this.dv(values[10])})`;
		console.log(qtext)
		pool.query(qtext)
			.then(res => {
				console.log(res);
	...
	UPDATE (question, callback) {
		...
		const values = [questionId, getSyntaxe(question.question_title), getSyntaxe(question.answer_title_a), getSyntaxe(question.answer_title_b), getSyntaxe(question.answer_title_c), getSyntaxe(question.answer_title_d), question.right_answer, getSyntaxe(question.explanation), question.difficulty, question.online_exam_ids, question.verified, question.quiz_details_id];
		const qtext = `UPDATE question_table SET question_title = ${this.dv(values[1])}, answer_title_a = ${this.dv(values[2])}, answer_title_b = ${this.dv(values[3])}, answer_title_c = ${this.dv(values[4])}, answer_title_d = ${this.dv(values[5])}, right_answer = ${this.dv(values[6])}, explanation = ${this.dv(values[7])}, difficulty = ${this.dv(values[8])}, online_exam_ids = ${this.dv(values[9])}, verified = ${this.dv(values[10])}, quiz_details_id = ${this.dv(values[11])} WHERE question_id = ${this.dv(values[0])}`;
		console.log(qtext)
		pool.query(qtext)
			.then(res => {
				console.log(res);
	...
	SELECT (criteria, callback) {
		let qtext = 'SELECT question_table.question_id as question_id, question_table.question_title as question_title, question_table.answer_title_a as answer_title_a, question_table.answer_title_b as answer_title_b, question_table.answer_title_c as answer_title_c, question_table.answer_title_d as answer_title_d, question_table.right_answer as right_answer, question_table.explanation as explanation, question_table.difficulty as difficulty, question_table.online_exam_ids as online_exam_ids, question_table.verified as verified FROM question_table'; // INNER JOIN HAS_RIGHT ON question_table.question_id = has_right.question_id INNER JOIN DBAUTHORIZATION ON has_right.authorizationId = dbauthorization.authorizationId
